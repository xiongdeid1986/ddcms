...
function ebcms_install(){
    ...
    // 执行sql
    \ebcms\Install::exec_sql_file(APP_PATH.'superpay/install/install.sql');

    // 导入与superpay相关的数据（配置 权限节点 表单 菜单）
    \ebcms\Install::imports('superpay');
...
function ebcms_uninstall(){
    ...
    // 执行sql
    \ebcms\Install::exec_sql_file(APP_PATH.'superpay/install/uninstall.sql');

    // 删除与superpay相关的数据（配置 权限节点 表单 菜单）
    \ebcms\Install::delete('superpay');
    return true;
}