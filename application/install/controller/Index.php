...
class Index extends Controller
{
    ...
    protected function step2()
    {
        if (request()->isGet()) {
            ...
            try{
                $pdo = new \PDO('mysql:dbname=' . $dbconfig['database'] . ';host=' . $dbconfig['hostname'] . ';port=' . $dbconfig['hostport'], $dbconfig['username'], $dbconfig['password']);
            }catch(\Exception $e){
                $this -> error('数据库配置错误');
            }

            \think\Session::set('dbdemo', $input['demodatabase']);
