...
function ebcms_install(){
    ...
    // 执行sql
    \ebcms\Install::exec_sql_file(APP_PATH.'link/install/install.sql');

    // 导入与link相关的数据（配置 权限节点 表单 菜单）
    \ebcms\Install::imports('link');
...
function ebcms_uninstall(){
    ...
    // 执行sql
    \ebcms\Install::exec_sql_file(APP_PATH.'link/install/uninstall.sql');

    // 删除与link相关的数据（配置 权限节点 表单 菜单）
    \ebcms\Install::delete('link');
    return true;
}